  ...
  return (
    <main className="flex flex-col lg:hidden">
      ...
      <div className="grid grid-cols-4 px-14.25 mb-20">
        ...
        <div className="col-span-4 grid grid-cols-3 justify-between h-100 mt-12 items-center justify-items-center">
          {logosOrdenados.map((Logo, index) => (
            // <Logo key={index} className="w-full h-auto " />
            <Logo
              key={index}
              className="max-w-16 md:max-w-24 lg:max-w-32 max-h-10"
            />
          ))}
        </div>
      </div>
